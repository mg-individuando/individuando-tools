 ...
 * - 13-16: Square icons
 *
 * Aspect ratios: 1,4,7,10 = wide (1033×213), 2,5,8,11 = portrait (466×440),
 *                3,6,9,12 = moderate (789×342), 13-16 = square (288×288)
 */
const INDIVIDUANDO_LOGOS = [
  ...
  { id: 13, label: "Ícone Cor", colors: ["#1e2f4c", "#c0d57e"], bgSuit: "light", aspect: "square" },
  { id: 14, label: "Ícone Verde+Branca", colors: ["#c0d57e", "#ffffff"], bgSuit: "dark", aspect: "square" },
  { id: 15, label: "Ícone Cor Alt.", colors: ["#1e2f4c", "#c0d57e"], bgSuit: "light", aspect: "square" },
  { id: 16, label: "Ícone Verde+Branca Alt.", colors: ["#c0d57e", "#ffffff"], bgSuit: "dark", aspect: "square" },
];
...
const LAYOUT_OPTIONS = [
  { value: "logo-text-logo" as const, label: "Logo | Texto | Logo", desc: "Layout clássico com logos nas laterais" },
  { value: "logo-text" as const, label: "Logo | Texto", desc: "Logo à esquerda com texto" },
  { value: "text-logo" as const, label: "Texto | Logo", desc: "Texto à esquerda com logo" },
...
async function recolorSvg(svgUrl: string, targetColor: string): Promise<string> {
  try {
    ...
    // Match hex colors in fill attributes and style blocks
...
/* ===================================================================
   ...
   =================================================================== */
function migrateConfig(config: Partial<BannerConfig>): BannerConfig {
...
export function BannerCreator({
  ...
  const autoDetect = useCallback(() => {
    ...
    setConfig((prev) => ({ ...prev, individuandoVariant: best }));
  }, [config.backgroundType, config.backgroundColor, config.gradientFrom, config.gradientTo, logoUrl]);
  ...
  function handleReset() {
    const fresh = migrateConfig(initialConfig || {});
    setConfig(fresh);
  }
  ...
  const clientLogoSrc = config.logoColorEnabled && recoloredClientLogo
    ? recoloredClientLogo
    : logoUrl;

  const individuandoLogoSrc = `/logos/individuando/logo-${config.individuandoVariant}.svg`;

  // Determine column arrangement based on layout
  function renderBannerPreview() {
    const bgStyle = getBackgroundStyle();

    const clientLogo = config.showClientLogo && clientLogoSrc ? (
      <div className="flex items-center justify-center shrink-0 px-3">
        <img
          src={clientLogoSrc}
          alt="Logo cliente"
          style={{
            height: `${Math.max(16, config.clientLogoSize * 0.5)}px`,
            maxWidth: "120px",
            objectFit: "contain",
          }}
        />
      </div>
    ) : config.showClientLogo && clientName ? (
      <div className="flex items-center justify-center shrink-0 px-3">
        <div
          className="rounded-lg flex items-center justify-center text-white font-bold"
          style={{
            width: `${Math.max(16, config.clientLogoSize * 0.5)}px`,
            height: `${Math.max(16, config.clientLogoSize * 0.5)}px`,
            fontSize: `${Math.max(10, config.clientLogoSize * 0.22)}px`,
    ...
    const individuandoLogo = (
      <div className="flex items-center justify-center shrink-0 px-3">
        <img
          src={individuandoLogoSrc}
          alt="Individuando"
          style={{
            height: `${Math.max(14, config.individuandoSize * 0.45)}px`,
            maxWidth: "110px",
            objectFit: "contain",
          }}
        />
        ...
        {config.title && (
          <p
            ...
            style={{
              ...
              fontSize: `${Math.max(10, config.titleSize * 0.5)}px`,
        ...
        {config.subtitle && (
          <p
            ...
            style={{
              ...
              fontSize: `${Math.max(8, config.subtitleSize * 0.5)}px`,
              textAlign: config.titlePosition,
            }}
          >
            {config.subtitle}
    ...
    switch (config.bannerLayout) {
      case "logo-text-logo":
        return (
          <div
            className="w-full rounded-lg overflow-hidden border shadow-sm transition-all duration-300 relative"
            style={{ aspectRatio: "1200 / 200", ...bgStyle }}
          >
            {config.overlayOpacity > 0 && <div style={overlayStyle} />}
            <div className="h-full flex items-center px-4 py-2 relative z-10">
              {config.clientLogoPosition === "left" ? (
                <>
                  {clientLogo}
                  {textBlock}
                  {individuandoLogo}
      ...
      case "centered":
        return (
          ...
          >
            {config.overlayOpacity > 0 && <div style={overlayStyle} />}
            <div className="h-full flex flex-col items-center justify-center px-4 py-2 relative z-10">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[92vh] overflow-hidden flex flex-col">
          ...
          <div className="px-6 pb-6 space-y-1">
            ...
            >
              <div className="space-y-3">
                <p className="text-xs text-gray-500">
                  Descreva o banner que você quer e a IA cria o design completo — cores, gradientes, layout, texto e escolha automática das logos.
                </p>
              ...
              <div className="space-y-4">
                ...
                {(config.bannerLayout === "logo-text-logo") && (
                  <div className="space-y-2">
                    <Label className="text-xs text-gray-500">Posição da logo do cliente</Label>
                    <div className="flex gap-2">
                    ...
                    <p className="text-[11px] text-gray-400">
                      A logo Individuando ficará no lado oposto automaticamente.
                    </p>
                  </div>
                )}
              </div>
                  ...
                  <>
                    {!logoUrl && (
                      <p className="text-xs text-amber-600 bg-amber-50 px-3 py-2 rounded-lg">
                        Nenhuma logo do cliente foi carregada. Faça upload na seção &ldquo;Logo do Cliente&rdquo; acima.
                      </p>
                    )}
            ...
            >
              <div className="space-y-4">
                ...
                {/* Subtitle */}
                <div className="space-y-2">
                  <Label className="text-xs text-gray-500">Subtítulo (opcional)</Label>
                  <Input
                ...
                {(config.title || config.subtitle) && (
                  <div className="space-y-3 pt-1">
                    <div className="flex items-center gap-4 flex-wrap">
                      <div className="flex items-center gap-2">
                        <Label className="text-xs text-gray-500">Cor título</Label>
                        <input
                          type="color"
                      ...
                      {config.subtitle && (
                        <div className="flex items-center gap-2">
                          <Label className="text-xs text-gray-500">Cor subtítulo</Label>
                          <input
                            type="color"
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t bg-gray-50">
          ...
          <Button
            onClick={handleSave}
            className="bg-[#1e2f4c] hover:bg-[#162340]"
          >
            Salvar Banner
          </Button>
        </div>
